...
async def correlate_deployment_async(deployment_id: int):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not analysis.get("has_metrics"):
            console.print(f"[yellow]⚠ No metrics data available for deployment #{deployment_id}[/yellow]")
        ...
        console.print(f"[dim]Deployed at: {deployment.deployed_at.strftime('%Y-%m-%d %H:%M:%S UTC')}[/dim]")
        console.print()
        ...
        metrics_order = ["error_rate", "request_rate", "latency_p50", "latency_p95", "cpu_usage", "memory_usage"]
        ...
        if issues:
            ...
            console.print("[dim]Run: gitops-audit show {} for full details[/dim]".format(deployment_id))
        else:
